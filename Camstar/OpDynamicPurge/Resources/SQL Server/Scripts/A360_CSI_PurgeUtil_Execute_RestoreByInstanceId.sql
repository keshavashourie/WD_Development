ALTER PROCEDURE CSI_PurgeUtil_Execute_RestoreByInstanceId 
...
BEGIN 
	...
	BEGIN TRY -- RAISERROR with severity 11-19 will cause execution to jump to the CATCH block.
        ...
        --PRINT @vRootTableName;
        --PRINT @vRootTableInstanceCol;
		---------------------------------------------------------------------------
        -- Get the latest BatchExecutionId based on instanceid
		---------------------------------------------------------------------------
		SET @vProgID = @vObject_Name + '.Get the latest BatchExecutionId based on instanceid';
        SET @vSQLStatement = N'SELECT @vBatchExecutionIdOUT = MAX(t.BatchExecutionId) FROM ' + @pvArchiveDBName + '.' + @pvArchiveSchemaName + '.' + @vRootTableName + ' t ';
        SET @vSQLStatement = @vSQLStatement + 'WHERE t.' + @vRootTableInstanceCol + ' = ''' + @pvInstanceId + ''''; 
        --PRINT @vSQLStatement;
        SET @vParmDefinition = N'@vBatchExecutionIdOUT NVARCHAR(16) OUTPUT';
    	EXECUTE sp_executesql @vSQLStatement 
            , @vParmDefinition
            , @vBatchExecutionIdOUT = @vBatchExecutionId OUTPUT;
        --PRINT @vPurgeId;
		---------------------------------------------------------------------------
        -- Restore using BatchExecutionId and InstanceId
		---------------------------------------------------------------------------
        --SET @vSQLStatement = 'EXECUTE CSI_PurgeUtil_Rest_' + @pvSetupName + ' @pvBatchExecutionId = ''' + @vBatchExecutionId + ''';';
        SET @vSQLStatement = 'EXECUTE CSI_PurgeUtil_Rest_' + @pvSetupName + ' @pvBatchExecutionId = ''' + @vBatchExecutionId + '''' + ', @pvInstanceId = ''' + @pvInstanceId + ''';';
        --PRINT @vSQLStatement
		SET @vProgID = @vObject_Name + '.' + @vSQLStatement;
        EXECUTE sp_executesql @vSQLStatement;
        --------------------------------------------------------------------------
		SET @vEndProcessTime = sysdatetime();
		SET @vProcessExecutionTime = DateDiff(millisecond, @vStartProcessTime, @vEndProcessTime);
        -- Log a completion message 
 	    SET @vMessage = @pvSetupName + ' Setup : Restore from BatchExecutionId ''' + @vBatchExecutionId + '''' + ' based on InstanceId ''' + @pvInstanceId + ''''
