IF NOT EXISTS (SELECT name FROM sysobjects WHERE name = 'DPT_IV_ContainerPurge' AND type = 'V')
    EXECUTE sp_executesql N'CREATE VIEW DPT_IV_ContainerPurge AS SELECT 1 AS dummy';
GO
ALTER VIEW DPT_IV_ContainerPurge 
AS
--===========================================================================
-- Author      : Ni Jia, Yeow
...
--     1. Lot Id is the Instance Id provided.
...
WITH vInitPurgeParent AS (SELECT TOP 50
	...
	WHERE 
		(
			...
			AND ctn.ContainerId IN (':vInstanceValue')
		)
    ORDER BY ctn.LastActivityDate, ctn.ContainerId
	--AND NOT ctn.SplitFromId IS NULL
	UNION ALL
    ...
    SELECT
		...
		LEVEL + 1 AS LEVEL,
		TREE
    FROM $(DatabaseName).$(SchemaName).Container par
    ...
    SELECT
		...
		LEVEL + 1 AS LEVEL,
		TREE
    FROM $(DatabaseName).$(SchemaName).Container child
