ALTER PROCEDURE CSI_PurgeUtil_TableSetup_SetupExists 
/* ---------------------------------------------------------------------------
  Description      : CSI_PurgeUtil_TableSetup_SetupExists
                     Check if the purging setup exists in the CSI_PURGEUTIL_SETUP table.
  Author           : Benny.Chia 
  Date             : 02 Apr 2014
...
BEGIN 
	...
	BEGIN TRY -- RAISERROR with severity 11-19 will cause execution to jump to the CATCH block.
		...
		BEGIN CATCH
			...
			SET @vProgID = @vObject_Name + '.' + 'Existence check on purging setup ' + @pvSetupName;
