...
CREATE VIEW V_ENHANCEDLOTHISTORY
AS
SELECT B.CONTAINERNAME "LOTID"
  , A.CDOName + CASE A.CallByCDOName WHEN A.CDOName THEN '' 
                ELSE ' (' + A.CallByCDOName + ')' END "SERVICE"
  , FORMAT(A.TxnDate, 'yyyy-MM-dd HH-mm-ss') "TXNDATE"
  , A.Username     "USER"
  ...
  , NULL           "REASONNAME"
  , NULL           "BONUS_REJ_QTY"
  , CASE A.STATUS WHEN 0 THEN 'DELETED' 
                  WHEN 1 THEN 'ACTIVE' 
                  WHEN 2 THEN 'TERMINATED' 
                  WHEN 3 THEN 'INTRANSIT' 
                  WHEN 4 THEN 'ISSUED' 
                  WHEN 5 THEN 'SHIPPED' 
                  ELSE 'UNDEFINED' END "LOTSTATUS"
  , NULL           "TARGETLOT"
  , NULL           "TARGETLOTQTY"
  , NULL           "TARGETLOTQTY2"
  , NULL           "SOURCELOT"
  ...
  , A.ContainerTxnSequence "LOTTXNSEQ"
FROM HistoryMainline A
  , Container B
WHERE A.HISTORYID = B.CONTAINERID
  AND (
    NOT (
...
SELECT B.CONTAINERNAME "LOTID"
  ...
  , A.Username       "USER"
  , A.FromSpecName   "FROMSTEP"
  , CASE ISNULL(D.ATTRIBUTENAME, 'XXX') WHEN 'Qty' THEN CAST(D.Oldvalue AS INT) ELSE NULL END  "FROMQTY"
  , CASE ISNULL(D.ATTRIBUTENAME, 'XXX') WHEN 'Qty2' THEN CAST(D.Oldvalue AS INT) ELSE NULL END "FROMQTY2"
  , CASE ISNULL(D.ATTRIBUTENAME, 'XXX') WHEN 'Qty' THEN CAST(D.Newvalue AS INT) ELSE NULL END  "QTY"
  , CASE ISNULL(D.ATTRIBUTENAME, 'XXX') WHEN 'Qty2' THEN CAST(D.Newvalue AS INT) ELSE NULL END "QTY2"
  , A.SpecName       "TOSTEP"
  , A.ResourceName   "EQUIPMENT"
  , A.ShiftName      "SHIFT"
...
WHERE F.Fromcontainerid = B.ContainerId
...
WHERE H.TOCONTAINERID = B.CONTAINERID
...
UNION
SELECT B.CONTAINERNAME "LOTID"
  , A.CDOName + CASE A.CallByCDOName WHEN A.CDOName THEN '' ELSE ' (' + A.CallByCDOName + ')' END
  , FORMAT(A.TxnDate, 'yyyy-MM-dd HH-mm-ss') "TXNDATE"
  , A.Username "USER"
  , A.FromSpecName "FROMSTEP"
  , A.FROMQTY      "FROMQTY"
  ...
  , CASE A.STATUS WHEN 0 THEN 'DELETED' WHEN 1 THEN 'ACTIVE' WHEN 2 THEN 'TERMINATED' WHEN 3 THEN 'INTRANSIT' WHEN 4 THEN 'ISSUED' WHEN 5 THEN 'SHIPPED' ELSE 'UNDEFINED' END "LOTSTATUS"
  , NULL     "TARGETLOT"
  , NULL     "TARGETLOTQTY"
  , NULL     "TARGETLOTQTY2"
  , NULL     "SOURCELOT"
...
SELECT B.CONTAINERNAME "LOTID"
  ...
  , FORMAT(getdate(), 'yyyy-MM-dd HH-mm-ss') "TXNDATE"
  , NULL   "USER"
  ...
  , NULL   "REASONNAME"
  , NULL   "BONUS_REJ_QTY"
  , CASE B.STATUS WHEN 0 THEN 'DELETED' 
                  WHEN 1 THEN 'ACTIVE' 
                  WHEN 2 THEN 'TERMINATED' 
                  WHEN 3 THEN 'INTRANSIT' 
                  WHEN 4 THEN 'ISSUED' 
...
FROM Container B
  , CURRENTSTATUS X
  , WORKFLOWSTEP  Y
WHERE X.CURRENTSTATUSID = B.CURRENTSTATUSID
  AND Y.WORKFLOWSTEPID = X.WORKFLOWSTEPID
;

/*
select *
from V_ENHANCEDLOTHISTORY
where LOTID = 'EPILOT04'
ORDER BY TXNDATE ASC, LOTTXNSEQ ASC
*/