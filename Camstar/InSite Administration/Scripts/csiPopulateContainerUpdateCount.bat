...
REM setlocal
REM --------------------------------------------------------------------------------------------------
REM  csiPopulateContainerUpdateCount.bat
...
REM  Usage: 
REM    csiPopulateContainerUpdateCount.bat <DB2/ORACLE/SQLSERVER> <Database> <user> <password
REM
...
ECHO @..\Scripts\oracle\ContainerUpdateCount.or.sql >> tempdata_%DB_NAME%.sql
...
SQLCMD -S %DB_SERVERNAME% -d %DB_NAME% -U %DB_USERNAME% -P %DB_PASSWORD% -b -i "..\Scripts\Sql Server\ContainerUpdateCount.sql"
IF ERRORLEVEL 1 set errorstatus=99
GOTO EXIT
:END
...
ECHO -----------------------------------------------------------------------------
ECHO USAGE:
ECHO   "csiPopulateEventUpdateData.bat <DB2|ORACLE|SQLSERVER> <Database Host> <Database Name> <User> <Password>"
ECHO Where
