...
REM setlocal
REM --------------------------------------------------------------------------------------------------
REM  csiReloadCMData.bat
REM  Simple batch file used to run the ReloadCMDefaultData scripts.
REM  Assumptions:
...
REM  Usage: 
REM    csiReloadCMData.bat <ORACLE/SQLSERVER> <Database> <user> <password
REM
REM Modification History:
REM Name			Date        	Action
REM --------------------------	----------	------------------------------------------------------------------------------------------
REM Vijaya Arya		02/20/2015      Created to reload the change management default seed data from Management Studio Load CM checkbox
REM
...
ECHO @Scripts\oracle\ReloadCMDefaultData.or.sql >> tempdata_%DB_NAME%.sql
...
SQLCMD -S %DB_SERVERNAME% -d %DB_NAME% -U %DB_USERNAME% -P %DB_PASSWORD% -b -i "Scripts\Sql Server\ReloadCMDefaultData.sql"
IF ERRORLEVEL 1 set errorstatus=99
GOTO EXIT
:END
...
ECHO -----------------------------------------------------------------------------
ECHO USAGE:
ECHO   "csiReloadCMData.bat <DB2|ORACLE|SQLSERVER> <Database Host> <Database Name> <User> <Password>"
ECHO Where
...
ECHO  Example :- "csiReloadCMData.bat ORACLE PROD insiteadmin admin"
ECHO		 "csiReloadCMData.bat SQLSERVER DBSERVERNAME PROD insiteadmin admin"
ECHO		 "csiReloadCMData.bat SQLSERVER 10.10.10.10 PROD insiteadmin admin"
ECHO ------------------------------------------------------------------------------
