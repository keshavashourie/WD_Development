...
namespace Camstar.WebPortal.Helpers
{
    ...
    /// </summary>
    public class scsContainerStatusInquiry : CommandBarHelper
    {
        public scsContainerStatusInquiry()
        {
        }
        ...
        public override PanelNameValueData GetPanelData(cbArgs callbackArgs)
        {
            ...
            if (panelData != null)
            {
                if (callbackArgs.fun == "getContainerInfo")
                {
                    ...
                    getContainerStatus(panelData, callbackArgs.containerName, callbackArgs.filter, callbackArgs.eProcedure, callbackArgs.tasklistName, callbackArgs.taskName, callbackArgs.tasklistId, callbackArgs.tasklistRevision, callbackArgs.tag);
                }
                ...
                else if (callbackArgs.fun == "openCheckSheetTasklistSummary")
                {
                    openCheckSheetTasklistSummary(panelData, callbackArgs.containerName, callbackArgs.eProcedure, callbackArgs.tasklistName, callbackArgs.taskName, callbackArgs.tasklistId, callbackArgs.tasklistRevision);
                }
                else
                {
                    //return
        ...
        //  Allow for potential override in derived classes
        /// <summary>
        /// 
        /// </summary>
        ...
        public virtual void getContainerStatus(PanelNameValueData panelData, string containerName, string filter, string eProcedure, string tasklistName = null, string taskName = null, string taskListId=null, string taskListRev = null, string tag = null)
        {
            ...
            if (filter == "all")
            {
                req.Info.CurrentContainerStatus = new OM.CurrentContainerStatus_Info
                {
                    ...
                    scsWIPEquipmentName = new OM.Info(true),
            ...
            {
                // Update to get the EquipmentName from WIPEquipment instead of CurrentStatus
                res.Value.CurrentContainerStatus.ResourceName = res.Value.CurrentContainerStatus.scsWIPEquipmentName;
                ...
                else if (filter == "documents")
                {
                    ...
                    {
                        var taskCDO = getTransactionTasksDetails(containerName, tasklistName, taskName, eProcedure, taskListId, taskListRev);
        ...
        private void openCheckSheetTasklistSummary(PanelNameValueData response, string containerName, string eProcedure, string tasklistName, string txnName, string taskListId, string taskListRev)
        {
            var txnTaskDetails = getTransactionTasksDetails(containerName, tasklistName, txnName, eProcedure, taskListId, taskListRev);
            if (txnTaskDetails != null)
            {
                ...
                response.Add("OpenCheckSheetTasklistSummary", new
                {
        ...
        private OM.ExecuteCheckSheet getTransactionTasksDetails(string containerName, string tasklistName, string txnName, string eProcedure, string taskListId, string taskListRev)
        {
            
            var frmSess = FrameworkManagerUtil.GetFrameworkSession();
            var svc = new ExecuteCheckSheetService(frmSess.CurrentUserProfile);

            var eProcName = eProcedure;
            var eProcRevision = "";

            if (eProcedure != null && eProcedure.Length > 0 && eProcedure.IndexOf(':') != -1)
            {

                var eProcedureArr = eProcedure.Split(':');
                eProcName = eProcedureArr[0];
                eProcRevision = eProcedureArr[1];
            }

            var parent = new OM.RevisionedObjectRef(tasklistName, taskListRev);
            var data = new OM.ExecuteCheckSheet { Container = new OM.ContainerRef(containerName), Task = new OM.NamedSubentityRef(txnName, parent), ElectronicProcedure = new OM.RevisionedObjectRef(eProcName, eProcRevision) };
            var req = new ExecuteCheckSheet_Request
            {
                Info = new OM.ExecuteCheckSheet_Info
                {
                    ...
                    ElectronicProcedure = new OM.Info(true)
                }
            };
            ExecuteCheckSheet_Result res;
