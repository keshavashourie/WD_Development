...
/// <summary>
/// Summary description for SS_LotTerminate
/// </summary>
namespace Camstar.WebPortal.WebPortlets.Shopfloor
{
	public class SS_LotTerminateEx : scsShopfloorBase
	{
		#region Properties

		protected CWC.TextBox _txtSelectionId { get { return Page.FindCamstarControl("LotTerminate_SelectionIdField") as CWC.TextBox; } }
		protected JQDataGrid _gridContainers { get { return Page.FindCamstarControl("ContainersField") as JQDataGrid; } }
		protected CWC.TextBox txtComputerName { get { return Page.FindCamstarControl("LotTerminate_ComputerNameField") as CWC.TextBox; } }
		private SEMI.AppCode.DataEnvelopControl _envSelectedLots { get { return Page.FindCamstarControl("LotTerminate_SelectedLotsList") as SEMI.AppCode.DataEnvelopControl; } }
        private Camstar.WebPortal.PortalFramework.ToggleContainer _togComments { get { return Page.FindCamstarControl("Control") as Camstar.WebPortal.PortalFramework.ToggleContainer; } }
		...
		public void SelectionIdField_DataChanged()
		{
			try
			{
				if (_txtSelectionId.Data != null)
				{
					...
					LotTerminateService objSvc = new LotTerminateService(fs.CurrentUserProfile);
					LotTerminate objSvcData = new LotTerminate { SelectionId = (string)_txtSelectionId.Data };
					LotTerminate_Info objSvcInfo = new LotTerminate_Info { Containers = FieldInfoUtil.RequestValue() };

					// init the result object
					LotTerminate_Result objResult = new LotTerminate_Result();

					// execute to request the value
					ResultStatus resultStatus = objSvc.ResolveSelectionId(objSvcData, new LotTerminate_Request { Info = objSvcInfo }, out objResult);
					...
					if (resultStatus.IsSuccess)
					{
						...
						foreach (OM.ContainerRef oContainer in objResult.Value.Containers)
						{
							...
							{
								SEMI.AppCode.UIUtility.GetLotQuerySelection(this, "LotTerminate", oContainer.Name.ToString(), false, ref _gridContainers, "ContainersField", true);
							}
						}
					}
					else
		...
		public override void GetInputData(Service serviceData)
		{
			...
			if (_gridContainers.Data != null)
			{
				...
				OM.LotTerminate svcData = serviceData as OM.LotTerminate;
			...
			if (Page.IsPostBack)
				...
				if (SEMI.AppCode.UIUtility.IsPopupClose(this))
				{
					if (_envSelectedLots != null)
						...
						if (Page.DataContract.GetValueByName("LotTerminate_SelectedLotsListDM") != null)
						{
							//gets the list of returned lot from the popup form
							_envSelectedLots.SS_ContainersList = Page.DataContract.GetValueByName("LotTerminate_SelectedLotsListDM") as string[];
						}
					...
					if (_envSelectedLots.SS_ContainersList != null)
					{
						...
						foreach (string sContainersItem in sContainers)
						{
							...
							if (string.IsNullOrEmpty(strSelectedGridId))
							{
								...
								SEMI.AppCode.UIUtility.GetLotQuerySelection(this, "LotTerminate", sContainersItem, false, ref _gridContainers, "ContainersField", true);
							}
						}
						//nullify the containers list
						_envSelectedLots.SS_ContainersList = null;
					}
                    var ContainersField = (Page.FindCamstarControl("ContainersField")) as JQDataGrid;

                    if (IsResponsive)
                    {
                        if (ContainersField.Settings.Automation == null)
                            ContainersField.Settings.Automation = new GridAutomation();

                        ContainersField.Settings.Automation.ShrinkColumnWidthToFit = false;
                    }
                }
		}
