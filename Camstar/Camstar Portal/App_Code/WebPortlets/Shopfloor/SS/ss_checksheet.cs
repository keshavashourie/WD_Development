...
{
    public class SS_CheckSheet : MatrixWebPart, IPostBackEventHandler
    {

        #region "Controls"

        protected ContainerListGrid ContainerName
        {
        ...
        protected MatrixWebPart TransactionAreaWP
        {
            get
            {
                ...
                {
                    if (Parent.Controls[z].ID == "TransactionAreaWP")
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (Page.Request.Form["__EVENTARGUMENT"] != null)
            {
                ...
                if (eventArgument.Contains("TxnConfirm"))
                {
                    Page.ActionDispatcher.ExecuteAction("ExecuteAction");
                }
            }
        ...
        /// <param name="e"></param>
        private void PagePanelChanged(object sender, EventArgs e)
        {
            if (WFControl != null)
            {
        ...
        private void TaskMenuChanged(object sender, EventArgs e)
        {
        ...
        public override void GetLineAssignment(OM.Service serviceData)
        {
            ...
            {
                (serviceData as OM.ExecuteCheckSheet).WorkCell = lineAssignmentResource as OM.NamedObjectRef;
            }
        ...
        public override bool PreExecute(OM.Info serviceInfo, OM.Service serviceData)
        {
            ...
            if (HiddenContainer != null && scrollCtrl != null)
            {
                if (HiddenContainer.Data != null && selectedTask.Data != null && selectedTaskList.Data != null)
                {
                    ...
                    if (serviceData.GetType() == typeof(OM.ExecuteCheckSheet))
                    {
                        switch (scrollCtrl.InstructionType)
                        {
                            case 1: //PassFail
                                (serviceData as OM.ExecuteCheckSheet).ParametricData = null;
                                (serviceData as OM.ExecuteCheckSheet).ParametricDataDef = null;
                                (serviceData as OM.ExecuteCheckSheet).DataCollectionDef = null;
                                break;

                            case 2: //Data Collection
                                (serviceData as OM.ExecuteCheckSheet).Pass = null;
                                break;
                        ...
                        (serviceData as OM.ExecuteCheckSheet).TaskList = parent;
                        (serviceData as OM.ExecuteCheckSheet).Task = taskItem;
            ...
            if (eventArgument.Contains("ExecuteCheckSheet"))
            {

                GetInputData(serviceData);
                ValidationStatus validationStatus = this.Page.ValidateInputData(serviceData);
                if (validationStatus.IsSuccess)
                {
                    ...
                    data.Container = Page.DataContract.GetValueByName<OM.ContainerRef>("SelectedContainerNameDM");
                    data.Task = (Page.DataContract.GetValueByName<OM.NamedSubentityRef>("TaskDM") ?? Page.DataContract.GetValueByName<OM.NamedSubentityRef>("EProcHiddenTaskDM"));
                    info.IsConfirmation = new OM.Info(true);

                    ExecuteCheckSheet_Request request = new ExecuteCheckSheet_Request { Info = info };

                    var Results = serv.Load(data, request, out resultData);

                    if (Results.IsSuccess)
                    {
                        if (resultData.Value != null && resultData.Value.IsConfirmation != null && resultData.Value.IsConfirmation.Value)
                            ScriptManager.RegisterStartupScript(TransactionAreaWP, this.GetType(), "MYConfirmScript", "CheckSheet_TxnConfirmation();", true);
                        else
                            return base.PreExecute(serviceInfo, serviceData);
                    }
                    else
                    {
                        Page.DisplayWarning(Results.Message);
                    }
                }
                else
                {
                    Page.DisplayWarning(validationStatus.Message);
                }
                return false;
            }
    ...
    } //public class CheckSheet : MatrixWebPart, IPostBackEventHandler
