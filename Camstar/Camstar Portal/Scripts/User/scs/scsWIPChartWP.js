...
var scsWIPChartWP = (function () {
    ...
    function initialize() {
        ...
        let $landingPage = getTabsByVPName("scsLandingPage_VP");
        $landingPage[0].on("click", function () {
            removeRejectChart();
            removeWIPChart();

            updateChart("ctl00_WebPartManager_scsWIPChartWP_RejectChart", null);
            updateChart("ctl00_WebPartManager_scsWIPChartWP_WipChart", null);
        });
    }
    ...
    function wipEmptyMessage() {
        if ($("#ctl00_WebPartManager_scsWIPChartWP_WipChart").find(".wlp-empty-state").length == 0) {
            var element = $('#ctl00_WebPartManager_scsWIPChartWP_WipChart').append("<div class='wlp-empty-state' id='lp-emp-state' />");
        ...
        $("#ctl00_WebPartManager_scsWIPChartWP_WipChart_ChartContainer").hide();
    }

    function rejectEmptyMessage() {
        if ($("#ctl00_WebPartManager_scsWIPChartWP_RejectChart").find(".rlp-empty-state").length == 0) {
            var element = $('#ctl00_WebPartManager_scsWIPChartWP_RejectChart').append("<div class='rlp-empty-state' id='lp-emp-state' />");
        ...
        $("#ctl00_WebPartManager_scsWIPChartWP_RejectChart_ChartContainer").hide();
    }
    ...
    function refreshBarChartAjax(type, clientCallback) {
        var transition = new Camstar.Ajax.Transition(eval(Camstar.Ajax.RequestType.Command), scsWIPChartWP);
        ...
        var communicator = new Camstar.Ajax.Communicator(transition, scsWIPChartWP);
    ...
    function refreshRejectChartClientCallback(data) {
        if (data.Data && data.Data.HTML) {
            ...
            updateChart("ctl00_WebPartManager_scsWIPChartWP_RejectChart", data.Data.HTML);
        } else {
            rejectEmptyMessage();
        }
    }
    ...
    function refreshWIPChartClientCallback(data) {
        if (data.Data && data.Data.HTML) {
            wipChartNotEmpty();
            removeWIPChart();
            updateChart("ctl00_WebPartManager_scsWIPChartWP_WipChart", data.Data.HTML);
        } else {        
            wipEmptyMessage();
        }
    }
    ...
    function updateChart(chatId, data) {
        ...
        chart[0].control.RefreshChart_HomePage('Multibar');
    }

    function removeRejectChart() {
        $("#ctl00_WebPartManager_scsWIPChartWP_RejectChart_ChartContainer svg:first").remove();
    }

    function removeWIPChart() {
        $("#ctl00_WebPartManager_scsWIPChartWP_WipChart_ChartContainer svg:first").remove();
    }

    function get_serverType() {
        return "Camstar.WebPortal.WebPortlets.Shopfloor.scsWIPChartWP, App_Code";
    }

    function get_controlId() {
        return "scsWIPChartWP";
    }
})();