...
(function () {
    ...
    var api = CR.GetNamespace('CR.isMfgOperationMaterials');
    ...
    function loadMaterialRequirements(resourceName, mfgOrder, hideNonRequirement) {
        ...
        if (mfgOrder && resourceName) {
            let request = {
                ...
                OperationId: mfgOrder.OperationId,
                MfgOrderQty: mfgOrder.MfgOrderQty
            };

            $.ajax({
                type: "POST",
                dataType: "json",
                url: './MfgOperationService.svc/web/LoadMaterialQueue',
                headers: {
    ...
    function populateResourceDetails(response) {
        ...
        if (!response.Success) {
            ...
            if (response.Components)
                _resourceDetailsComponents = response.Components;

            if (response.MaterialList)
                _resourceDetailsMaterialList = response.MaterialList;
        ...
        if (response) {
            if (_resourceDetailsComponents.length > 0) {
                ...
                if (document.getElementById('Lot'))
                    document.getElementById('Lot').innerHTML = 'Container/Lot (Qty)';
                if (document.getElementById('Slot'))
                document.getElementById('Slot').innerHTML = 'Slot/Position';
                for (var i = 0; i < _resourceDetailsComponents.length; i++) {
                    let name = getValue(_resourceDetailsComponents[i].Lot) + "_" + getValue(_resourceDetailsComponents[i].Product) + "_" + getValue(_resourceDetailsComponents[i].SlotName) + "_" + getValue(_resourceDetailsComponents.SubSlotName);
                    _resourceDetailsComponents[i].RowId = name;
                    if (_resourceDetailsComponents[i].QtyRequired) {
                        addRowToGrid(_resourceDetailsComponents[i].Lot + "_" + _resourceDetailsComponents[i].Product,
                            getValue(_resourceDetailsComponents[i].SlotName), _resourceDetailsComponents[i].Product, _resourceDetailsComponents[i].Revision,
                            getValue(_resourceDetailsComponents[i].Lot) + "(" + _resourceDetailsComponents[i].QtyRequired + ")", getValue(_resourceDetailsComponents[i].ProducingOrder), _resourceDetailsComponents[i].BomQty, 0, true, getValue(_resourceDetailsComponents.SubSlotName));
                    }
                    else {
                        addRowToGrid(_resourceDetailsComponents[i].Lot + "_" + _resourceDetailsComponents[i].Product,
                            getValue(_resourceDetailsComponents[i].SlotName), _resourceDetailsComponents[i].Product, _resourceDetailsComponents[i].Revision,
                            getValue(_resourceDetailsComponents[i].Lot), getValue(_resourceDetailsComponents[i].ProducingOrder), _resourceDetailsComponents[i].BomQty, 0, true, getValue(_resourceDetailsComponents.SubSlotName));
                    }
            ...
            if (_resourceDetailsMaterialList.length > 0) {
                ...
                for (var i = 0; i < _resourceDetailsMaterialList.length; i++) {
                    addRowToGrid(_resourceDetailsMaterialList[i].ContainerName + "_" + _resourceDetailsMaterialList[i].PartNumber,
                        getValue(_resourceDetailsMaterialList[i].SlotName), _resourceDetailsMaterialList[i].PartNumber, _resourceDetailsMaterialList[i].Revision,
                        _resourceDetailsMaterialList[i].ContainerName, getValue(_resourceDetailsMaterialList[i].ProducingOrder), _resourceDetailsMaterialList[i].QtyRequired * _mfgOrder.MfgOrderQty, 0, true, getValue(_resourceDetailsMaterialList[i].SubSlotName));
                }
            ...
            if (_hideNonRequirement)
                mfgOperationMaterials.removeNonRequirement();
        }
        else
    ...
    function addRowToGrid(rowId, slotName, productName, revision, identifier, producingOrder, qty, qtyRequired, isLot, subSlotName) {
        let table = getElement(SELECTORS.materialsGrid);
        let rowNum = table.find("tbody > tr").length + 1;
        let prodNameRev = revision ? productName + ":" + revision : productName;
        ...
        var newRowMarkup = MATERIAL_TEMPLATES.materialRow
            ...
            .replace(new RegExp('_PRODUCT_', 'g'), prodNameRev ? prodNameRev : '')
    ...
    function validateRequirementFulfill(components, materialList) {
        ...
        for (var index in components) {
            ...
            if (comp.Product && comp.Product !== materialList.PartNumber && !comp.Satisfied && !comp.PartiallySatisfied) {
            ...
            else if (comp.PartiallySatisfied) {

                cssClass = "requirement-unfulfill";
                title = _labels['PartiallySatisfied'];
                fromMaterial = false;
                $compRow.attr('from-material', fromMaterial);
            }
            else {
