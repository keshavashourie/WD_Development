...
(function($) {
	...
	*/
    ...
    /* fires postback when date was changed and datepicker is closed */
    $.datepicker._doChange = function (event)
    {
        var inst = $.datepicker._getInst(event.target);
        if (inst.settings.autopostback)
        {
            if ($.datepicker._datepickerShowing)
                $.datepicker._needPostback = true;
            else
    ...
    $.datepicker._base_hideDatepicker = $.datepicker._hideDatepicker;
    $.datepicker._hideDatepicker = function (input)
    {
        $.datepicker._base_hideDatepicker(input);
        var inst = this._curInst;
        if (inst && $.datepicker._needPostback)
        {
            $.datepicker._needPostback = false;
            __doPostBack(inst.input[0].id, '');
        }
    };
