...
SET @PermissionModesFlag_NoSecAdmin = 2
	...
	PRINT('Creating mdTeamTracking Menu...')
	EXEC createPortalMenuDefinition 'mdTeamTracking', 'Team Tracking pages used in the Portal', @DefaultNotes, @MenuDefId OUTPUT
	EXEC createPortalMenuItem @MenuDefId, 7835, 10, 'mdWeb_ManageTeamEmployees', 'Manage Team Employees', 'mdManageTeamEmployees_VP', '', ''
	EXEC createPortalMenuItem @MenuDefId, 7835, 20, 'mdWeb_WorkAreaSignInSignOut', 'Work Area Sign In', 'mdWorkAreaSignInSignOut_VP', '', ''

	PRINT('Creating mdTeamTracking Menu into V8 PortalMenu...')
	SELECT @MenuDefId = PortalMenuDefinitionId
	FROM PortalMenuDefinition
	WHERE PortalMenuDefinitionName = 'csiPortalMenuV8';

	EXEC createPortalMenuItem @MenuDefId, 7833, 05, 'mdWeb_TeamTracking', 'Team Tracking',  '', '', 'mdTeamTracking'

	PRINT('Creating mdTeamTracking Menu into MobileMenu...')
	SELECT @MenuDefId = PortalMenuDefinitionId
	FROM PortalMenuDefinition
	...
	PRINT('Creating mdTeamTracking Menu into existing PortalMenu...')
	SELECT @MenuDefId = PortalMenuDefinitionId
	FROM PortalMenuDefinition
    ...
    EXEC csiRBACCreatePermission @RoleId, 'Device Identifier Maint', 180, 3760, @PermissionModesFlag_NoSecAdmin
    EXEC csiRBACCreatePermission @RoleId, 'UDI Expression Maint', 180, 7140, @PermissionModesFlag_NoSecAdmin
	...
	EXEC csiRBACCreatePermission @RoleId, 'mdWorkAreSignSignOut_VP', 110, 7755, @PermissionModesFlag_None
	EXEC csiRBACCreatePermission @RoleId, 'mdManageTeamEmployees_VP', 110, 8143, @PermissionModesFlag_None
    ...
    EXEC csiRBACCreatePermissionsForQry @RoleId, @PermissionSQL, @PermissionModesFlag_None
	EXEC csiRBACCreatePermission @RoleId, 'Add Team Members', 110, 3760, @PermissionModesFlag_ReadOnly
	EXEC csiRBACCreatePermission @RoleId, 'Remove Team Members', 110, 3760, @PermissionModesFlag_ReadOnly
	EXEC csiRBACCreatePermission @RoleId, 'Work Area Sign In', 110, 3760, @PermissionModesFlag_ReadOnly
	EXEC csiRBACCreatePermission @RoleId, 'Work Area Sign Out', 110, 3760, @PermissionModesFlag_ReadOnly
...
INSERT INTO SecurityCacheRefreshRequest (SecurityCacheRefreshRequestId, CreatedDate, CreatedDateGMT, EntityName, EntityType) VALUES (48,GETDATE(),GETUTCDATE(),N'Default Mfg',2)
