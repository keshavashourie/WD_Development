...
--
-- Copyright © 1995-2009, Camstar Systems, Inc. All Rights Reserved.
...
BEGIN
  ...
  vDefaultNotes := 'This menu is created by the install process.  Best practice is to copy this menu and modify the copy, instead of modifying this menu directly.';
   --'mdTeamTracking Menu...'
	 createPortalMenuDefinition('mdTeamTracking', 'Team Tracking pages used in the Portal', vDefaultNotes, vMenuDefId);
	 createPortalMenuItem (vMenuDefId, 7835, 10, 'mdWeb_ManageTeamEmployees', 'Manage Team Employees', 'mdManageTeamEmployees_VP', '', '' );
	 createPortalMenuItem (vMenuDefId, 7835, 20, 'mdWeb_WorkAreaSignInSignOut', 'Work Area Sign In', 'mdWorkAreaSignInSignOut_VP', '', '' );

	  --'Creating MenuItem into existing Portal Menu...'
  SELECT PortalMenuDefinitionId
  INTO vMenuDefId
  FROM PortalMenuDefinition
  WHERE PortalMenuDefinitionName = 'csiPortalMenu';
  createPortalMenuItem(vMenuDefId, 7833, 10, 'mdWeb_TeamTracking', 'Team Tracking', '', '', 'mdTeamTracking');

   --'Creating MenuItem into existing Portal Menu V8...'
	SELECT PortalMenuDefinitionId
	INTO vMenuDefId
	FROM PortalMenuDefinition
	...
	createPortalMenuItem(vMenuDefId, 7833, 10, 'mdWeb_TeamTracking', 'Team Tracking', '', '', 'mdTeamTracking');
	    ...
	    --'Creating MenuItem into existing Portal Mobile Menu ...'
	SELECT PortalMenuDefinitionId
	INTO vMenuDefId
	FROM PortalMenuDefinition
    ...
    csiRBACCreatePermission (vRoleId, 'Add Team Members', 110, 3760, vPermissionModesFlag_NoSecAdmn);
    csiRBACCreatePermission (vRoleId, 'Remove Team Members', 110, 3760, vPermissionModesFlag_NoSecAdmn);
	csiRBACCreatePermission (vRoleId, 'Work Area Sign In', 110, 3760, vPermissionModesFlag_NoSecAdmn);
	csiRBACCreatePermission (vRoleId, 'Work Area Sign Out', 110, 3760, vPermissionModesFlag_NoSecAdmn);

SELECT RoleId INTO vRoleId FROM RoleDef WHERE Rolename = 'Default Modeling';
    vPermissionSQL := '110';
    csiRBACCreatePermissionsForQry (vRoleId, vPermissionSQL, vPermissionModesFlag_None);
    csiRBACCreatePermission (vRoleId, 'Device Identifier Maint', 180, 3760, vPermissionModesFlag_NoSecAdmn);
    csiRBACCreatePermission (vRoleId, 'UDI Expression Maint', 180, 7140, vPermissionModesFlag_NoSecAdmn);
  ...
  csiRBACCreatePermission (vRoleId, 'mdWorkAreSignSignOut_VP', 110, 7755, vPermissionModesFlag_None);
	csiRBACCreatePermission (vRoleId, 'mdManageTeamEmployees_VP', 110, 8143, vPermissionModesFlag_None);
	csiRBACCreatePermission (vRoleId, 'DeviceIdentifier_VP', 110, 7755, vPermissionModesFlag_None);
	csiRBACCreatePermission (vRoleId, 'UDIExpression_VP', 110, 8143, vPermissionModesFlag_None);
...
INSERT INTO SecurityCacheRefreshRequest (SecurityCacheRefreshRequestId, CreatedDate, CreatedDateGMT, EntityName, EntityType) VALUES (48,SYSTIMESTAMP,SYS_EXTRACT_UTC(SYSTIMESTAMP),N'Default Mfg',2);
...
UPDATE EMPLOYEE SET  PortalMenuDefinitionId = (SELECT PortalMenuDefinitionId FROM portalmenuDefinition WHERE PortalMenuDefinitionName = 'csiPortalMenu')
