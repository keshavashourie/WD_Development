/*
-- SCRIPT:isPopulateDefaultUserQuery.or.sql
...
--------------------------------------------------------------------------------
-- PROCEDURE: isSTInstall_NewUserQuery
-- DESCR: 
--
*/
--#delimiter
CREATE OR REPLACE PROCEDURE isSTInstall_NewUserQuery(pName VARCHAR2
...
/*
-- PROCEDURE: isST_PopulateDefaultUQData
-- DESCR:
--
*/
--#delimiter
CREATE OR REPLACE PROCEDURE isST_PopulateDefaultUQData
AS
   VSQLString CLOB;
BEGIN
    vSQLString := 'Select
...
WHERE 
    ...
    isSTInstall_NewUserQuery('isRecipePlan_Selection','Do not delete. Default Query used by Recipe Plan.','Do not delete.',vSQLString,0,'isRecipePlan',5,'Resource',5,'Spec',5);
   
END;
--#delimiter
BEGIN
   isST_PopulateDefaultUQData;
   COMMIT;
END;
--#delimiter
BEGIN
    EXECUTE IMMEDIATE 'DROP PROCEDURE isST_PopulateDefaultUQData';
    EXECUTE IMMEDIATE 'DROP PROCEDURE isSTInstall_NewUserQuery';
END;
--#delimiter
...
BEGIN
    ...
    vSQLString := 'SELECT CalendarDate as CALENDARDATE, '||
	...
	'WHERE TXNDATE>=SYSDATE-90';
    
	csiSTInstall_NewDef('ISRVOEERAWDETAILS', 'Used for the ISOEERAWDETAILS.', NULL, vSQLString, 1, 'ISRVOEERAWDETAILS', 0, NULL, NULL, '0,', NULL, 1,1); 
    
    vSQLString := 'SELECT c.BatchComments as BATCHCOMMENTS, '||
	...
	'WHERE cs.LASTMOVEDATE>= SYSDATE-90';
    
	csiSTInstall_NewDef('ISRVWIP', 'Used for the WIP Container data.', NULL, vSQLString, 1, 'ISRVWIP', 0, NULL, NULL, '0,', NULL, 1,1); 
    

    vSQLString := 'SELECT CAST(DPHD.DATAVALUE AS DECIMAL) AS VALUE , '||
	'DPHD.DATANAME , '||
	...
	'AND HML.TXNDATE>= SYSDATE-90';
    
	csiSTInstall_NewDef('ISRVDATAPOINTHISTORY', 'Used for the Data Point History.', NULL, vSQLString, 1, 'ISRVDATAPOINTHISTORY', 0, NULL, NULL, '0,', NULL, 1,1); 
	...
	' WHERE COMPLETEDDATE>= SYSDATE-90';
    
	csiSTInstall_NewDef('ISRVCOMPLETWIPSUMM', 'Used for the Completed WIP SUmmary.', NULL, vSQLString, 1, 'ISRVCOMPLETWIPSUMM', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'CASE ' || 
'WHEN NVL(TRAININGSUBTRAININGREQREV, '''') = '''' ' || 
...
'CASE ' || 
'WHEN NVL(TRAININGRECORDEXPIRATIONDATE, TO_DATE(''1900-01-01'', ''YYYY-MM-DD'')) = TO_DATE(''1900-01-01'', ''YYYY-MM-DD'') ' || 
'THEN ''NOT ASSIGNED'' ' || 
'WHEN TRAININGRECORDEXPIRATIONDATE > SYSDATE THEN ''VALID'' ' || 
'ELSE ''NOT VALID'' ' || 
'END AS "TRAINING RECORD", ' || 
'TRAININGRECORDEXPIRATIONDATE, ' || 
...
'NVL(TRAININGREQUIREMENTBASE.TRAININGREQUIREMENTNAME, SUBTRAININGREQUIREMENTBASE.TRAININGREQUIREMENTNAME) AS TRAININGSUBTRAININGREQUIREMENT, ' || 
'NVL(TRAININGREQUIREMENT.TRAININGREQUIREMENTREVISION, SUBTRAININGREQUIREMENT.TRAININGREQUIREMENTREVISION) AS TRAININGSUBTRAININGREQREV, ' || 
...
'AND ' || 
'(REPLACE(SUBTRAININGPLANDETAIL.TRAININGREQUIREMENTID, ''0000000000000000'', SUBTRAININGREQUIREMENTBASE.REVOFRCDID) = SUBTRAININGREQUIREMENT.TRAININGREQUIREMENTID ' || 
...
;    
	csiSTInstall_NewDef('TRAININGPLAN', 'Training plan', NULL, vSQLString, 1, 'TRAININGPLAN', 0, NULL, NULL, '0,', NULL, 1,1); 
    
vSQLString :='SELECT TRAININGREQUIREMENTBASE.TRAININGREQUIREMENTNAME AS "TRAININGREQUIREMENT", ' || 
'TRAININGREQUIREMENTBASE.TRAININGREQUIREMENTNAME || '' ('' || TRAININGREQUIREMENT.TRAININGREQUIREMENTREVISION || '')'' AS "TRAININGREQUIREMENT(REV)", ' || 
...
'FROM ' || 
'EMPLOYEE TRAINERS ' || 
'RIGHT OUTER JOIN TRAININGREQUIREMENTTRAINERS ON (TRAINERS.EMPLOYEEID = TRAININGREQUIREMENTTRAINERS.TRAINERSID) ' || 
...
csiSTInstall_NewDef('TRAINERS', 'Training Requirement Trainers', NULL, vSQLString, 1, 'TRAINERS', 0, NULL, NULL, '0,', NULL, 1,1); 
...
csiSTInstall_NewDef('TRAINING_RECORDS', 'TRAINING RECORDS', NULL, vSQLString, 1, 'TRAINING_RECORDS', 0, NULL, NULL, '0,', NULL, 1,1);

vSQLString := 'SELECT DISTINCT ' ||
'    A.*, ' ||
'    B.CONTAINERNAME, ' ||
'    B.MFGORDERNAME, ' ||
'    B.QTY, ' ||
...
'        CAST(CONTAINER.ORIGINALQTY AS VARCHAR2(255)) || '' '' || UOM.UOMNAME AS ORIGINALQTY, ' ||
'        CASE ' ||
'            WHEN SUM(NVL(CONTAINER.QTY, 0)) = 0 THEN ''0 '' || UOM.UOMNAME ' ||
'            ELSE CAST(SUM(NVL(CONTAINER.QTY, 0)) AS VARCHAR2(255)) || '' '' || UOM.UOMNAME ' ||
'        END AS CURRENTQTY, ' ||
'        CONTAINER.CONTAINERID ' ||
'    FROM MFGORDER ' ||
'    RIGHT OUTER JOIN CONTAINER ON CONTAINER.MFGORDERID = MFGORDER.MFGORDERID ' ||
'    LEFT OUTER JOIN UOM ON CONTAINER.UOMID = UOM.UOMID ' ||
'    GROUP BY CONTAINER.CONTAINERNAME, MFGORDER.MFGORDERNAME, MFGORDER.QTY, MFGORDER.PLANNEDSTARTDATE, ' ||
'    MFGORDER.PLANNEDCOMPLETIONDATE, CONTAINER.ORIGINALQTY, UOM.UOMNAME, CONTAINER.CONTAINERID ' ||
...
'    WHERE REPLACE(PRODUCT.BOMID, ''0000000000000000'', PRODUCTBOMBASE.REVOFRCDID) = PRODUCTBOM.BOMID ' ||
'    AND REPLACE(PRODUCTBOM.BOMBASEID, ''0000000000000000'', PRODUCTBOM.BOMBASEID) = PRODUCTBOMBASE.BOMBASEID ' ||
'    ) C ON A.CONTAINERID = C.CONTAINERID ' ||
' ORDER BY B.CONTAINERNAME ASC';

csiSTInstall_NewDef('ACTIVITYLOG_CT', 'ACTIVITYLOG CT', NULL, vSQLString, 1, 'ACTIVITYLOG_CT', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'    CAST((ISSUEACTUALSHISTORY.QTY - COALESCE(REMOVEHISTORYDETAIL2.QTYREMOVED, 0)) AS VARCHAR2(255)) || '' '' || UOM3.UOMNAME AS ISSUEDQTYEACH, ' ||
'    CAST(ISSUEHISTORYDETAIL.QTYREQUIRED AS VARCHAR2(255)) || '' '' || UOM3.UOMNAME AS REQUIREDQTYEACH, ' ||
'    HISTORYMAINLINE.REVERSALSTATUS ' ||
' FROM ' ||
'    ESIGHISTORYDETAIL ' ||
...
'    CONTAINER2.CONTAINERNAME ASC';

csiSTInstall_NewDef('MATERIALISSUED_CT', 'MATERIALISSUED CT', NULL, vSQLString, 1, 'MATERIALISSUED_CT', 0, NULL, NULL, '0,', NULL, 1,1); 
...
vSQLString :=  'SELECT DISTINCT ' ||
    ...
    'CASE ' ||
        ...
        'ELSE PRODUCTBASE3.PRODUCTNAME || '' ('' || PRODUCT3.PRODUCTREVISION || '')'' ' ||
    'END AS PRODUCTREVISION, ' ||
    'REMOVEHISTORYDETAIL.QTYREMOVED || '' '' || REMOVEDQUANTITYUOM.UOMNAME AS QTYREMOVED, ' ||
...
'WHERE ' ||
    '( ' ||
        'UPPER(CDODEFINITION.CDONAME) = ''COMPONENTREMOVE'' ' ||
        'AND (HISTORYMAINLINE.CONTAINERID = HISTORYCROSSREF.TRACKINGID) ' ||
    ') ' ||
'ORDER BY ' ||
    'CONTAINER.CONTAINERNAME ASC, ' ||
    'REMOVALCONTAINER.CONTAINERNAME ASC';

csiSTInstall_NewDef('MATERIALREMOVED_CT', 'MATERIALREMOVED CT', NULL, vSQLString, 1, 'MATERIALREMOVED_CT', 0, NULL, NULL, '0,', NULL, 1,1);
...
csiSTInstall_NewDef('GENERAL_ED', 'GENERAL_ED', NULL, vSQLString, 1, 'GENERAL_ED', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'EVENTLOT.QTY, ' ||
'EVENTLOT.UOMNAME, ' ||
'EVENTLOT.QTY - NVL((SELECT SUM(ED.QTY)FROM EVENTDISPOSITION ED INNER JOIN EVENTLOTEVENTDISPOSITIONS ELED ON ED.EVENTDISPOSITIONID = ELED.DISPOSITIONSID WHERE ELED.EVENTLOTID = EVENTLOT.EVENTLOTID), 0) AS QTYPENDINGDISP, ' ||
...
csiSTInstall_NewDef('AMD_ED', 'AMD_ED', NULL, vSQLString, 1, 'AMD_ED', 0, NULL, NULL, '0,', NULL, 1,1);
...
csiSTInstall_NewDef('CHECKLIST_ED', 'CHECKLIST_ED', NULL, vSQLString, 1, 'CHECKLIST_ED', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'CASE EVENTFAILURECAUSE.ISROOTCAUSE WHEN 1 THEN ''TRUE'' WHEN 0 THEN ''FALSE'' END AS "ISROOTCAUSE", EVENTFAILURECAUSE.COMMENTS AS "CAUSECOMMENTS" ' ||
...
csiSTInstall_NewDef('FMI_ED', 'FMI_ED', NULL, vSQLString, 1, 'FMI_ED', 0, NULL, NULL, '0,', NULL, 1,1);
...
'ROLE.ROLENAME, ' ||
'APPROVER.EMPLOYEENAME AS "NAME", ' ||
'APPROVERBY.EMPLOYEENAME AS "APPROVEDBY", ' ||
...
csiSTInstall_NewDef('RES_ED', 'RES_ED', NULL, vSQLString, 1, 'RES_ED', 0, NULL, NULL, '0,', NULL, 1,1);
...
'CSDH.SAMPLINGSTATUS, ' ||
'CAST(CSDH.TXNDATEGMT AS TIMESTAMP) AS TRANSACTIONTIME, ' ||
'CSTDH.DATAVALUE, ' ||
'LREASON.LOSSREASONNAME, ' ||
'INSPECTIONLEVEL.INSPECTIONLEVELNAME, ' ||
'AQLLEVEL.AQLLEVELNAME, ' ||
'EMPLOYEE.EMPLOYEENAME, ' ||
'PLANBASE.SAMPLINGPLANNAME ' ||
'FROM CONTAINER ' ||
'LEFT JOIN HISTORYCROSSREF ' ||
...
'INNER JOIN COLLECTSAMPLINGDATAHISTORY CSDH ' ||
'ON CSDH.HISTORYMAINLINEID = HISTORYMAINLINE.HISTORYMAINLINEID ' ||
'LEFT JOIN MFGLOT ' ||
'ON CSDH.SAMPLINGLOTID = MFGLOT.MFGLOTID ' ||
'INNER JOIN SAMPLINGPLAN PLAN ' ||
'ON MFGLOT.SAMPLINGPLANID = PLAN.SAMPLINGPLANID ' ||
'INNER JOIN SAMPLINGPLANBASE PLANBASE ' ||
'ON PLAN.SAMPLINGPLANBASEID = PLANBASE.SAMPLINGPLANBASEID ' ||
'INNER JOIN SAMPLETEST ' ||
'ON CSDH.SAMPLETESTID = SAMPLETEST.SAMPLETESTID ' ||
'INNER JOIN SAMPLETESTBASE ' ||
'ON SAMPLETEST.SAMPLETESTBASEID = SAMPLETESTBASE.SAMPLETESTBASEID ' ||
'INNER JOIN AQLLEVEL ' ||
'ON AQLLEVEL.AQLLEVELID = CSDH.AQLLEVELID ' ||
'INNER JOIN INSPECTIONLEVEL ' ||
'ON INSPECTIONLEVEL.INSPECTIONLEVELID = CSDH.INSPECTIONLEVELID ' ||
'INNER JOIN COLLECTSAMPLINGHISTORYDETAILS CSHD ' ||
'ON CSDH.COLLECTSAMPLINGDATAHISTORYID = CSHD.HISTORYDETAILSID ' ||
'INNER JOIN COLLECTSAMPLINGTESTDTLHISTORY CSTDH ' ||
'ON CSHD.COLLECTSAMPLINGHISTORYDETAILID = CSTDH.HISTORYDETAILSID ' ||
'LEFT JOIN LOSSREASON LREASON ' ||
'ON CSTDH.REJECTREASONID = LREASON.LOSSREASONID';

csiSTInstall_NewDef('SAMPLINGDETAILS_LSH', 'SAMPLINGDETAILS_LSH', NULL, vSQLString, 1, 'SAMPLINGDETAILS_LSH', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'INSPECTIONLEVEL.INSPECTIONLEVELNAME, ' ||
'CSDH.SAMPLINGSTATUS, ' ||
'MAX(CAST(CSDH.TXNDATEGMT AS TIMESTAMP)) AS LASTTRANSACTIONTIME, ' ||
'SUM(CSDH.TOTALREJECTS) AS TOTALREJECTS, ' ||
'SUM(CSDH.TOTALSAMPLESCOMPLETED - CSDH.TOTALREJECTS) AS SAMPLESPASSED, ' ||
'SUM(CSDH.REQUIREDSAMPLES) AS TOTALREQUIREDSAMPLES, ' ||
'SUM(CSDH.TOTALSAMPLESCOMPLETED) AS TOTALSAMPLESCOMPLETED, ' ||
'PLANBASE.SAMPLINGPLANNAME ' ||
'FROM CONTAINER ' ||
'LEFT JOIN HISTORYCROSSREF ' ||
'ON HISTORYCROSSREF.TRACKINGID = CONTAINER.CONTAINERID ' ||
...
'INSPECTIONLEVEL.INSPECTIONLEVELNAME, ' ||
'CSDH.SAMPLINGSTATUS, ' ||
'CAST(CSDH.TXNDATEGMT AS TIMESTAMP), ' ||
'PLANBASE.SAMPLINGPLANNAME';

csiSTInstall_NewDef('SAMPLINGSUMMARY_LSH', 'SAMPLINGSUMMARY_LSH', NULL, vSQLString, 1, 'SAMPLINGSUMMARY_LSH', 0, NULL, NULL, '0,', NULL, 1,1);
...
'CSTDH.LOWERLIMIT, ' ||
'CSTDH.UPPERLIMIT, ' ||
'CASE WHEN CSTDH.PASSED = ''1'' THEN ''Passed'' ELSE ''Failed'' END AS TESTRESULT, ' ||
'AQLLEVEL.AQLLEVELNAME, ' ||
'INSPECTIONLEVEL.INSPECTIONLEVELNAME, ' ||
'SDPB.SAMPLEDATAPOINTNAME, ' ||
'EMPLOYEE.EMPLOYEENAME, ' ||
'PLANBASE.SAMPLINGPLANNAME ' ||
'FROM CONTAINER ' ||
'LEFT JOIN HISTORYCROSSREF ' ||
...
'INNER JOIN COLLECTSAMPLINGTESTDTLHISTORY CSTDH ' ||
'ON CSHD.COLLECTSAMPLINGHISTORYDETAILID = CSTDH.HISTORYDETAILSID ' ||
'LEFT JOIN SAMPLEDATAPOINT SDP ' ||
'ON CSTDH.SAMPLEDATAPOINTID = SDP.SAMPLEDATAPOINTID ' ||
'LEFT JOIN SAMPLEDATAPOINTBASE SDPB ' ||
'ON SDPB.SAMPLEDATAPOINTBASEID = SDP.SAMPLEDATAPOINTBASEID';

csiSTInstall_NewDef('SAMPLEDTLS_LSH', 'SAMPLEDTLS_LSH', NULL, vSQLString, 1, 'SAMPLEDTLS_LSH', 0, NULL, NULL, '0,', NULL, 1,1); 
...
'FROM RESOURCEDEF LOTRESOURCEDEF ' ||
'RIGHT OUTER JOIN LOTSAMPLEDATA ' ||
'ON LOTSAMPLEDATA.RESOURCEID = LOTRESOURCEDEF.RESOURCEID ' ||
'INNER JOIN MFGLOT ' ||
'ON LOTSAMPLEDATA.SAMPLINGLOTID = MFGLOT.MFGLOTID ' ||
...
'INNER JOIN SPEC LOTSPEC ' ||
'ON LOTSAMPLEDATA.SPECID = LOTSPEC.SPECID ' ||
'INNER JOIN SPECBASE LOTSPECBASE ' ||
'ON LOTSPEC.SPECBASEID = LOTSPECBASE.SPECBASEID ' ||
'INNER JOIN AQLLEVEL LOTAQLLEVEL ' ||
'ON LOTSAMPLEDATA.AQLLEVELID = LOTAQLLEVEL.AQLLEVELID ' ||
'INNER JOIN INSPECTIONLEVEL LOTINSPECTIONLEVEL ' ||
'ON LOTSAMPLEDATA.INSPECTIONLEVELID = LOTINSPECTIONLEVEL.INSPECTIONLEVELID ' ||
'INNER JOIN SAMPLETEST LOTSAMPLETEST ' ||
'ON LOTSAMPLEDATA.SAMPLETESTID = LOTSAMPLETEST.SAMPLETESTID ' ||
'INNER JOIN SAMPLETESTBASE LOTSAMPLETESTBASE ' ||
'ON LOTSAMPLETEST.SAMPLETESTBASEID = LOTSAMPLETESTBASE.SAMPLETESTBASEID ' ||
'LEFT OUTER JOIN VENDORITEM LOTVENDORITEM ' ||
'ON LOTSAMPLEDATA.VENDORITEMID = LOTVENDORITEM.VENDORITEMID';

csiSTInstall_NewDef('LOTSAMPLINGDATA_LSH', 'LOTSAMPLINGDATA_LSH', NULL, vSQLString, 1, 'LOTSAMPLINGDATA_LSH', 0, NULL, NULL, '0,', NULL, 1,1);
...
' LOTSAMPLINGPLANBASE.SAMPLINGPLANNAME ' ||
'FROM PRODUCTBASE LOTSAMPLEPRODUCTBASE ' ||
' INNER JOIN PRODUCT LOTSAMPLEPRODUCT ON LOTSAMPLEPRODUCT.PRODUCTBASEID = LOTSAMPLEPRODUCTBASE.PRODUCTBASEID ' ||
...
csiSTInstall_NewDef('MFGLOT_LSH', 'MFGLOT_LSH', NULL, vSQLString, 1, 'MFGLOT_LSH', 0, NULL, NULL, '0,', NULL, 1,1);
